...
func CollectApiCheckRuns(taskCtx plugin.SubTaskContext) errors.Error {
	...
	// Query commits from raw PR data collected in this run to ensure truly incremental collection
	...
	// Check if we have any existing check runs for this repo
	// If not, do a full collection even if incremental mode is enabled
	var existingCount int64
	existingCount, err = db.Count(dal.From("_tool_github_check_runs"),
	...
	taskCtx.GetLogger().Info("Querying PR commits for check runs from raw data in repo_id=%d, connection_id=%d, incremental=%v",
